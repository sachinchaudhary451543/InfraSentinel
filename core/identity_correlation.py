...
class IdentityCorrelationService:
    ...
    def correlate_agent_payload(tenant_id: int, server_id: int, hostname: str, serial_number: str, logged_in_user: str) -> None:
        ...
        # Match the office email first, then tolerate local-account naming
        # differences such as ``sachinkumar`` vs ``sachin.kumar``.
        employee = Employee.query.filter(
            Employee.tenant_id == tenant_id,
            Employee.email.ilike(clean_user),
        ).first() if '@' in clean_user else None
        if not employee:
            candidates = Employee.query.filter_by(tenant_id=tenant_id).all()
            compact_user = IdentityCorrelationService._compact_identity(clean_user)
            employee = next((candidate for candidate in candidates if (
                IdentityCorrelationService._compact_identity(candidate.local_username) == compact_user
                or IdentityCorrelationService._compact_identity(candidate.email) == compact_user
            )), None)
        if employee and '@' in clean_user:
            # Replace only generated placeholder addresses; never overwrite a
            # client-maintained employee email with agent telemetry.
            if not employee.email or employee.email.endswith('@unknown.local'):
                employee.email = clean_user
            employee.local_username = employee.local_username or clean_user.split('@', 1)[0]
            db.session.commit()
        if not employee:
