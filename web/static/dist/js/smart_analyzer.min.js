            ...
            ${columns
              .map(
                ...
                <td class="p-3 border text-sm">${
                  row[col] !== null && row[col] !== undefined ? row[col] : "ΓÇö"
                }</td>
            `
              )
              .join("")}
...
document.getElementById("uploadForm")?.addEventListener("submit", async (e) => {
  ...
  if (!fileInput?.files[0]) {
    showMessage("uploadMessage", "Γ¥î Please select a file", "error");
    return;
  }
  ...
  try {
    ...
    if (data.error) {
      showMessage("uploadMessage", `Γ¥î ${data.error}`, "error");
      return;
    }
    currentUID = data.id;
    currentData = data;
    ...
    showMessage(
      "uploadMessage",
      `Γ£à File analyzed successfully! ${data.summary.rows} records processed.`,
      "success"
    );
    ...
    showMessage("uploadMessage", `Γ¥î Error: ${err.message}`, "error");
  } finally {
    showLoading(false);
  }
});
document
  ...
  ?.addEventListener("click", async () => {
    ...
    try {
      ...
      showMessage(
        "uploadMessage",
        "Γ£à Current database analyzed successfully!",
        "success"
      );
