...
def api_server_screenshots(server_id):
    ...
    active_user = request.args.get('active_user', '').strip()
    ...
    if active_user:
        q = q.filter(db.func.lower(Screenshot.active_user) == active_user.lower())
...
def agent_metrics():
    ...
    try:
        ...
        office_user_id = (data.get('office_user_id') or logged_in_user or '').strip()
        local_username = (data.get('local_username') or logged_in_user or '').strip()
        identity_user = office_user_id or local_username
        ...
        logger.info(f"Parsed: user={identity_user}, app={active_app}, idle={idle_time_seconds}")
        ...
        if identity_user:
            ...
            logger.info(f"Employee activity added: user={identity_user}")
        ...
        if ss_data and ss_data.get('success') and ss_data.get('image'):
            try:
                ...
                shot.active_user    = identity_user
        ...
        # Evaluate the persisted telemetry after its transaction succeeds. This
...
def api_employee_productivity():
    from web.models import Employee, EmployeeActivity, Server
    import csv
    import io
    ...
    def compact_identity(value):
        return (value or '').split('@', 1)[0].lower().replace('.', '').replace('_', '').replace('-', '')

    # Canonicalize pre-UPN telemetry such as ``SachinKumar`` to the employee's
    # configured office email, so management reports do not split one person
    # into multiple rows after an agent identity upgrade.
    employee_query = Employee.query
    if not current_user.is_superadmin:
        employee_query = employee_query.filter_by(tenant_id=current_user.tenant_id)
    canonical_user_by_alias = {}
    for employee in employee_query.all():
        canonical = employee.email or employee.local_username or employee.name
        for alias in (employee.email, employee.local_username, employee.name):
            if alias:
                canonical_user_by_alias[compact_identity(alias)] = canonical

    # Aggregate per-day + per-user
    agg = {}
    for r in rows:
        day = r.timestamp.date().isoformat()
        reported_user = r.user or ''
        canonical_user = canonical_user_by_alias.get(compact_identity(reported_user), reported_user)
        key = (day, canonical_user)
        if key not in agg:
            agg[key] = {'date': day, 'user': canonical_user, 'records': 0, 'active_records': 0, 'idle_sum': 0.0, 'apps': {}}
        rec = agg[key]
        rec['records'] += 1
...
@api_bp.route('/api/v2/server/<int:server_id>/load-insight', methods=['GET'])
@login_required
def api_server_load_insight(server_id):
    """Return the latest observed load causes from agent process telemetry."""
    from web.models import db, Metric, Server
    server = db.session.get(Server, server_id)
    if not server or (not current_user.is_superadmin and server.tenant_id != current_user.tenant_id):
        return jsonify({'success': False, 'error': 'Unauthorized'}), 403
    metric = Metric.query.filter_by(server_id=server_id).order_by(Metric.timestamp.desc()).first()
    if not metric:
        return jsonify({'success': True, 'has_process_data': False, 'processes': [], 'message': 'No telemetry available yet.'})
    try:
    ...
    return jsonify({
        'success': True,
        'timestamp': metric.timestamp.isoformat() + 'Z' if metric.timestamp else None,
        'cpu': round(float(metric.cpu_util_percent or metric.cpu or 0), 1),
        'ram': round(float(metric.ram_util_percent or metric.ram or 0), 1),
        'active_app': details.get('active_app') or None,
        'window_title': details.get('window_title') or None,
        'processes': details.get('top_processes') or [],
        'has_process_data': bool(details.get('top_processes')),
    })


@api_bp.route('/api/v2/server/<int:server_id>/remote/process/stop', methods=['POST'])
@login_required
def api_stop_remote_process(server_id):
    """Queue a confirmed process stop for an admin/operator, never execute inline."""
    from web.models import db, Server, RemoteCommand, AuditLog
    server = db.session.get(Server, server_id)
    if not server or (not current_user.is_superadmin and server.tenant_id != current_user.tenant_id):
        return jsonify({'success': False, 'error': 'Unauthorized'}), 403
    if not (current_user.is_superadmin or current_user.role in {'super_admin', 'org_admin', 'tenant_admin'}):
    ...
    try:
        ...
        return jsonify({'success': False, 'error': 'A valid process ID is required'}), 400
    if pid <= 4 or pid > 4_000_000:
        return jsonify({'success': False, 'error': 'Process ID is outside the allowed range'}), 400
    process_name = str(data.get('process_name') or 'unknown')[:120]
    command = f"Stop-Process -Id {pid} -Force"
    command_row = RemoteCommand()
    command_row.server_id = server_id
    command_row.command = command
    command_row.status = 'pending'
    command_row.created_by = current_user.username
    db.session.add(command_row)
    audit = AuditLog(tenant_id=server.tenant_id, user=current_user.username, action='REMOTE_PROCESS_STOP', resource=f'Server:{server.hostname}', details=f'{process_name} (PID {pid})', status='pending', timestamp=datetime.utcnow())
    db.session.add(audit)
    db.session.commit()
    return jsonify({'success': True, 'command_id': command_row.id, 'message': f'Stop request queued for {process_name} (PID {pid}).'})


@api_bp.route('/api/v2/metrics/latest-batch', methods=['POST'])
@login_required
def api_metrics_latest_batch():
    """Return latest metrics for at most 100 tenant-visible servers in one query."""
    from sqlalchemy import and_, func
    from web.models import db, Metric, Server

    payload = request.get_json(silent=True) or {}
    raw_ids = payload.get('server_ids') or []
    if not isinstance(raw_ids, list):
        return jsonify({'success': False, 'error': 'server_ids must be a list'}), 400
    try:
        server_ids = list(dict.fromkeys(int(value) for value in raw_ids))[:100]
    except (TypeError, ValueError):
        return jsonify({'success': False, 'error': 'server_ids must contain numeric IDs'}), 400
    if not server_ids:
        return jsonify({'success': True, 'metrics': {}})

    visible_servers = db.session.query(Server.id).filter(Server.id.in_(server_ids))
    if not current_user.is_superadmin:
        visible_servers = visible_servers.filter(Server.tenant_id == current_user.tenant_id)
    visible_ids = [row.id for row in visible_servers.all()]
    if not visible_ids:
        return jsonify({'success': True, 'metrics': {}})
    ...
    ).filter(Metric.server_id.in_(visible_ids)).group_by(Metric.server_id).subquery()
    rows = db.session.query(Metric).join(
        latest,
        and_(Metric.server_id == latest.c.server_id,
             Metric.timestamp == latest.c.latest_timestamp)
    ).all()
    metrics = {
        str(row.server_id): {
            'success': True,
            'timestamp': row.timestamp.isoformat() + 'Z' if row.timestamp else None,
            'cpu': round(float(row.cpu_util_percent or row.cpu or 0), 1),
            'ram': round(float(row.ram_util_percent or row.ram or 0), 1),
            'disk': round(float(row.ssd_util_percent or row.disk or 0), 1),
        }
        for row in rows
    }
    for server_id in visible_ids:
        metrics.setdefault(str(server_id), {
            'success': True, 'timestamp': None, 'cpu': 0, 'ram': 0, 'disk': 0, 'no_data': True
        })
...
def api_server_forecast(server_id):
    ...
    try:
        ...
        from web.services.forecasting import HealthForecaster
        return jsonify(HealthForecaster.get_server_forecast(server_id)), 200
...
def api_server_alerts(server_id):
    ...
    try:
        active_alerts = db.session.query(SystemAlert).filter_by(
            server_id=server_id,
            is_active=True
