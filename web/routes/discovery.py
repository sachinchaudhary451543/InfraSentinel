...
from web.models import SystemDiscovery, Server, db
...
@discovery_bp.route('/api/v2/domain/discover', methods=['GET', 'POST'])
@login_required
@require_tenant_access
def api_discover_domain():
    """Async API endpoint to scan domain for systems using core.domain_discovery module"""
    domain_name = request.args.get('domain') or request.form.get('domain') or None
    company_tenant = request.args.get('company') or request.form.get('tenant') or None
    secret = request.args.get('secret') or request.form.get('secret') or None
    use_azure = request.args.get('use_azure') == '1' or request.form.get('use_azure') == '1'
    ...
    try:
        ...
        if systems:
            ...
            db.session.commit()
            return jsonify({'success': True, 'count': len(systems), 'message': f'Successfully discovered systems.'})
        else:
            return jsonify({'success': False, 'error': 'No systems found in domain.'})
    except Exception as e:
        logging.error(f"Domain discovery error: {e}")
...
def register_azure_tenant():
    ...
    try:
        ...
        # Step 2: Automatically map or provision SharePoint per-tenant architecture
        try:
            from sharepoint_provisioner import ensure_site_exists, provision_tenant
            site_url = ensure_site_exists("https://graph.microsoft.com", token)
            provision_tenant(site_url, token)
            flash(f'Azure tenant registered and SharePoint storage provisioned at {site_url}.', 'success')
        except Exception as sp_err:
            logging.error(f"SharePoint provisioning skipped/failed for tenant {tenant.id}: {sp_err}")
            flash('Azure tenant registered successfully (Note: SharePoint auto-provisioning could not complete or is pending Admin Consent).', 'info')
...
def import_discovered(discovery_id):
    ...
    discovery = db.get_or_404(SystemDiscovery, discovery_id)
