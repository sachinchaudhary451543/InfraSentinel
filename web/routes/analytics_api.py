...
def workforce_dashboard():
    ...
    def _render_error_page(message):
        logger.exception('Workforce dashboard rendering failed')
        return render_template(
            ...
            total_assignments=0,
            attendance_count=0,
            total_active_str='00:00:00',
            total_idle_str='00:00:00',
            total_productive_str='00:00:00',
            today=datetime.utcnow().date().isoformat(),
            total_agents=0,
            active_agents=0,
            idle_agents=0,
            offline_agents=0,
            screenshot_enabled_count=0,
            active_screens=0,
            error=message,
        ), 200
    ...
    try:
        ...
        for emp in employees:
            ...
            if assignment and assignment.server_id:
                ...
                if srv:
                    ...
                    device_ip = srv.ip or ''
                    screenshot_enabled = bool(getattr(srv, 'screenshot_enabled', False))
        ...
        for srv in servers:
            ...
            idle_sec = sum(s.idle_minutes or 0 for s in emp_sessions)
            prod_sec = sum(s.productive_minutes or 0 for s in emp_sessions)
            latest_ts = _safe_datetime(getattr(latest_activity, 'timestamp', None)) if latest_activity else None
            if not emp_sessions and latest_ts and day_start <= latest_ts <= day_end:
                idle_sec = int(getattr(latest_activity, 'idle_time', 0) or 0)

            live_agents.append({
                'id': emp.id if emp else None,
                'name': (
                    emp.display_name if emp and getattr(emp, 'display_name', None)
                    else getattr(latest_activity, 'user', None) if latest_activity and getattr(latest_activity, 'user', None)
                ...
                'screenshot_thumb': screenshot_thumb,
                'server_status': getattr(srv, 'status_label', 'offline'),
                'server_last_seen': _last_seen_label(getattr(srv, 'last_seen', None)),
                'active_str': _hms(active_sec),
                'idle_str': _hms(idle_sec),
                'productive_str': _hms(prod_sec),
                'current_app': getattr(latest_activity, 'app', '') if latest_activity else '',
                'current_window': getattr(latest_activity, 'window_title', '') if latest_activity else '',
            })
        ...
        screenshot_enabled_count = sum(1 for s in servers if bool(getattr(s, 'screenshot_enabled', False)))
