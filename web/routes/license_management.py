...
@license_bp.route('/user/<int:user_id>/history', methods=['GET'])
@login_required
def get_user_license_history(user_id):
    """Return the user's current assignments plus persisted assignment events."""
    if not current_user.is_superadmin:
        return jsonify({'error': 'Admin only'}), 403
    try:
        from web.models import db, AzureUser, AzureLicense, AzureLicenseAssignment, LicenseHistory
        ...
        current = db.session.query(AzureLicenseAssignment, AzureLicense).join(
            AzureLicense, AzureLicenseAssignment.license_id == AzureLicense.id
        ).filter(
            AzureLicenseAssignment.user_id == user.id,
            AzureLicenseAssignment.tenant_id == tenant_id,
        ).order_by(AzureLicenseAssignment.assigned_at.desc()).all()

        events = LicenseHistory.query.filter(
            LicenseHistory.tenant_id == tenant_id,
            LicenseHistory.user_id == user.user_id,
        ).order_by(LicenseHistory.event_date.desc()).limit(200).all()

        history = [{
            'event': event.event_type or 'ASSIGNMENT',
            'license': event.sku_part_number or event.sku_id or 'Unknown license',
            'date': event.event_date.isoformat() if event.event_date else None,
            'source': event.assignment_source or 'graph_sync',
        } for event in events]
        for assignment, license_obj in current:
            if not any(item['license'] == (license_obj.sku_name or license_obj.product_name) and item['event'] == 'ASSIGNED' for item in history):
                history.append({
                    'event': 'CURRENT',
                    'license': license_obj.sku_name or license_obj.product_name or 'Unknown license',
        ...
        return jsonify({
            'success': True,
            'user': {'id': user.id, 'name': user.display_name, 'email': user.email, 'department': user.department},
            'current_count': len(current),
            'history': sorted(history, key=lambda item: item.get('date') or '', reverse=True),
        })
    except Exception as e:
        logger.error(f"Error getting license history for user {user_id}: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
