...
class Server(db.Model):
    ...
    __table_args__ = (
        db.Index('idx_server_tenant_hostname', 'tenant_id', 'hostname'),
    )
...
class Metric(db.Model):
    ...
    __table_args__ = (
        db.Index('idx_metric_server_timestamp', 'server_id', 'timestamp'),
    )
...
class EmployeeActivity(db.Model):
    ...
    __table_args__ = (
        ...
        db.Index('idx_employee_activity_user_timestamp', 'user', 'timestamp'),
    )
...
class Screenshot(db.Model):
    ...
    __table_args__ = (
        db.Index('idx_screenshot_tenant_captured', 'tenant_id', 'captured_at'),
        db.Index('idx_screenshot_server_captured', 'server_id', 'captured_at'),
    )
...
class RemoteCommand(db.Model):
    ...
    __table_args__ = (
        db.Index('idx_remote_command_server_status_created', 'server_id', 'status', 'created_at'),
    )
...
class Employee(db.Model):
    ...
    # Local OS username to map agent data (e.g. "jsmith" -> "jsmith@example.com")
...
class ActivitySession(db.Model):
    ...
    __table_args__ = (
        db.Index('idx_activity_session_employee_start', 'employee_id', 'start_time'),
    )
